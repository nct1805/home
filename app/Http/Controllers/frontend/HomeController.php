    ...
    public function index(Request $request)
    {
		...
		if(!empty($getCate_for_popup)){
			foreach($getCate_for_popup as $k => $val){
				$getCate_for_popup[$k]['alias'] = VnToEn($val['title'], '/[\'^£$%&*()/}{@#~?><>,|=_+¬]/');
			}
		}
        ...
        $url_5giay = config('constants.url_5giay');
		$product_seen = '';
        ...
        return view('frontend.home.home', ['menu' => $menu, 'slide' => $slide, 'banner' => $banner, 'url_5giay' => $url_5giay, 'data' => $data, 'product_seen' => $product_seen, 'meta_title' =>$meta_title, 'meta_keyword' => $meta_keyword, 'meta_description' => $meta_description, 'meta_script' => $meta_script, 'isOnline' => $isOnline, 'getCate_for_popup' => $getCate_for_popup,'get_base_url' =>$get_base_url]);
    }
